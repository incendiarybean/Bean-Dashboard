...
function Component(props){
    ...
    if(!props.Notes.Loaded){
        ...
        return (
            <div style={{ backgroundImage: `url('${props.Theme.background}')` }} id='notes_container' onDoubleClick={(e) => props.Notes.createNote(e)} className='bg-coolGray-200 dark:bg-coolGray-700 notes-container relative bg-blue-500 shadow-inner hidden lg:block w-full h-full'>
                {props.Notes.Note.map((data) => (
                    <Note key={data._id} {...props} data={data} />
                ))}
                <div className='bg-coolGray-200 dark:bg-coolGray-700 p-1 group absolute border-accent border-b-4 shadow rounded-t-md top-2 left-2 flex w-auto justify-between'>
