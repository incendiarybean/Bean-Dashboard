...
function Component(props){
    ...
    if(!props.Notes.Loaded){
        ...
        return (
            <div id='notes_container' onDoubleClick={(e) => props.Notes.createNote(e)} className='bg-coolGray-200 dark:bg-coolGray-700 animate__animated animate__fadeIn notes-container relative bg-blue-500 shadow-inner hidden lg:block w-full h-full'>
                {props.Notes.Note.map((data) => (
                    <Note key={data._id} {...props} data={data} />
                ))}
                <div className="animate__animated animate__fadeIn absolute p-1 transform scale-125 rounded-full m-3" onClick={e => props.Notes.createNote({ target: document.getElementById('notes_container') , clientX: 800, clientY:150 })}>
